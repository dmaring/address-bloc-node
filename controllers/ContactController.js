...
module.exports = class ContactController {
  ...
  binarySearch(contacts, target){
    // contacts list must be sorted!!
    // declare and assign min and max
    let min = 0;
    ...
    while(min <= max) {
      ...
      // target is before the element at mid, eleminate upper bound
      if(currentContact.name > target) {
        max = mid - 1;
      // target is after the element at mid, elemenate lower bound
